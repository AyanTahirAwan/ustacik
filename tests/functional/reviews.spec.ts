...
async function createCompletedJob(customerId: number, craftsmanId: number, categoryId: number, regionId: number) {
  return JobRequest.create({ requestId: `REQ-${Date.now()}-${Math.random()}`, customerId, craftsmanId, categoryId, regionId, description: 'Test completed job', status: 'completed' })
}

test.group('Group 1: Reviews - CRUD Operations', (group) => {
  group.each.setup(() => testUtils.db().withGlobalTransaction())

  test('GET /api/craftsmen/:craftsmanId/reviews - Public listing includes the review', async ({ client }) => {
    ...
    response.assertBodyContains({ reviews: [{ jobId: job.id }], count: 1 })
  })

  test('GET /api/craftsmen/:craftsmanId/reviews - No average shown until 3 reviews exist', async ({ client, assert }) => {
    const data = await createTestData()
    const job = await createCompletedJob(data.customer.id, data.craftsman.id, data.category.id, data.region.id)
    await Review.create({ jobId: job.id, customerId: data.customer.id, craftsmanId: data.craftsman.id, punctuality: 5, workmanship: 5, priceHonesty: 5, communication: 5, comment: 'Great' })
  ...
  test('POST /api/reviews - Customer can submit a review for a completed job', async ({ client }) => {
    const data = await createTestData()
    const job = await createCompletedJob(data.customer.id, data.craftsman.id, data.category.id, data.region.id)

    const response = await client.post('/api/reviews').json({
      jobId: job.id,
      punctuality: 5,
      workmanship: 4,
      priceHonesty: 5,
  ...
  test('PATCH /api/reviews/:id/reply - Craftsman can reply to their review', async ({ client }) => {
    ...
    const response = await client.patch(`/api/reviews/${review.id}/reply`).json({ message: 'Thank you' }).loginAs(data.craftsman)
    response.assertStatus(200)
  ...
  test('POST /api/reviews/:id/helpful - Customer can mark a review helpful', async ({ client }) => {
    ...
    const otherCustomerUser = await User.create({ email: 'voter@test.com', phoneNormalised: '+1000000009', passwordHash: 'password123', role: 'customer', status: 'active' })
    await Customer.create({ userId: otherCustomerUser.id, fullName: 'Voter', language: 'en', smsOptIn: true })

    const response = await client.post(`/api/reviews/${review.id}/helpful`).loginAs(otherCustomerUser)
    response.assertStatus(201)
  })
  ...
  test('DELETE /api/admin/reviews/:id - Admin can delete a review', async ({ client }) => {
    ...
    const response = await client.delete(`/api/admin/reviews/${review.id}`).loginAs(data.admin)
...
test.group('Group 2: Reviews - Authorization & Ownership', (group) => {
  ...
  test('Customer cannot reply to a review', async ({ client }) => {
    ...
    const response = await client.patch(`/api/reviews/${review.id}/reply`).json({ message: 'Nope' }).loginAs(data.customer)
    response.assertStatus(403)
  })
  ...
  test('Wrong craftsman cannot reply to review', async ({ client }) => {
    ...
    await Craftsman.create({ userId: otherCraftsman.id, businessName: 'Other Craftsman', categoryId: data.category.id, trustLevel: 1, verbalConsent: true, totalJobs: 0 })

    const response = await client.patch(`/api/reviews/${review.id}/reply`).json({ message: 'Fake' }).loginAs(otherCraftsman)
  ...
  test('Non-admin cannot delete reviews', async ({ client }) => {
    ...
    const response = await client.delete(`/api/admin/reviews/${review.id}`).loginAs(data.customer)
    response.assertStatus(403)
  })
})
...
test.group('Group 4: Reviews - Edge Cases', (group) => {
  ...
  test('Craftsman cannot reply twice', async ({ client }) => {
    ...
    const response = await client.patch(`/api/reviews/${review.id}/reply`).json({ message: 'Second reply' }).loginAs(data.craftsman)
    response.assertStatus(400)
  })
  ...
  test('Duplicate helpful vote is rejected', async ({ client }) => {
    ...
    const voter = await User.create({ email: 'voter2@test.com', phoneNormalised: '+1000000010', passwordHash: 'password123', role: 'customer', status: 'active' })
    await Customer.create({ userId: voter.id, fullName: 'Voter Two', language: 'en', smsOptIn: true })

    await client.post(`/api/reviews/${review.id}/helpful`).loginAs(voter)
    const response = await client.post(`/api/reviews/${review.id}/helpful`).loginAs(voter)
